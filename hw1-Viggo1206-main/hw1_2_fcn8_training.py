...
def train(model, train_loader, optimizer, epoch, log_interval=100, start_time="", device="cuda"):
    ...
    for batch_idx, (data, labels, file_num) in enumerate(train_loader):  # for 每個 mini batch
        ...
        mask = make_mask(mask)
        Image.fromarray(mask).save(f"./fcn8/{start_time}/train/{ep}/{file_num[0]}.png")
...
def test(model, testset_loader, ep, start_time, device="cuda"):
    ...
    with torch.no_grad(): # This will free the GPU memory used for back-prop
        for batch_idx, (data, labels, file_num) in enumerate(testset_loader):
            ...
            mask = np.argmax(mask, axis=0)
            mask = make_mask(mask)
            Image.fromarray(mask).save(f"./fcn8/{start_time}/valid/{ep}/{file_num[0]}.png")
...
if __name__ == '__main__':
    ...
    train_set = MyDataset(root="./p2_data/train", transform=t)
    val_set = MyDataset(root="./p2_data/validation", transform=t)
    ...
    print(start_time)
    try:
        os.mkdir(f'./fcn8/{start_time}')
        os.mkdir(f'./fcn8/{start_time}/train')
        os.mkdir(f'./fcn8/{start_time}/valid')
    ...
    for ep in range(epoch):
        te1 = time.time()
        try:
            os.mkdir(f'./fcn8/{start_time}/train/{ep}')
            os.mkdir(f'./fcn8/{start_time}/valid/{ep}')
        except Exception as e:
            print(e)

        train_loss = train(model, train_loader, optimizer, ep, 100, start_time, device=device)
        save_checkpoint(f"./checkpoint/{start_time}-fcn8-{ep}.pth", model, optimizer)
